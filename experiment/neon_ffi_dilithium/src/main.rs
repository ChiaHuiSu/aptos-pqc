...
fn main() {
    ...
    unsafe {
        ...
        for i in 0..n {
            let key_res = PQCLEAN_DILITHIUM3_AARCH64_crypto_sign_keypair(pks[i].as_mut_ptr(), sks[i].as_mut_ptr());
        }
        ...
        for i in 0..n {
            let res = PQCLEAN_DILITHIUM3_AARCH64_crypto_sign_signature(
                sigs[i].as_mut_ptr(),
                &mut siglens[i],
                messages[i].as_ptr(),
                messages[i].len(),
        ...
        for i in 0..n {
            let verify_result = PQCLEAN_DILITHIUM3_AARCH64_crypto_sign_verify(
                sigs[i].as_ptr(),
                siglens[i],
                messages[i].as_ptr(),
                messages[i].len(),
