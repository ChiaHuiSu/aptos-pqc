...
unsafe extern "C" {
    ...
    fn pqcrystals_dilithium3_ref_keypair(
        pk: *mut u8,
        sk: *mut u8,
    ) -> i32;

    fn pqcrystals_dilithium3_ref_signature(
        sig: *mut c_uchar,
        siglen: *mut size_t,
        m: *const c_uchar,
        mlen: size_t,
...
fn main() {
    ...
    let n = 10000;
    let mut pks: Vec<[u8; CRYPTO_PUBLICKEYBYTES]> = vec![[0u8; CRYPTO_PUBLICKEYBYTES]; n];
    let mut sks: Vec<[u8; CRYPTO_SECRETKEYBYTES]> = vec![[0u8; CRYPTO_SECRETKEYBYTES]; n];
    let mut sigs: Vec<[u8; CRYPTO_BYTES]> = vec![[0u8; CRYPTO_BYTES]; n];
    let mut siglens: Vec<size_t> = vec![0; n];
    ...
    unsafe {
        ...
        for i in 0..n {
            let key_res = pqcrystals_dilithium3_ref_keypair(pks[i].as_mut_ptr(), sks[i].as_mut_ptr());
        }
        ...
        for i in 0..n {
            let res = pqcrystals_dilithium3_ref_signature(
                sigs[i].as_mut_ptr(),
                &mut siglens[i],
                messages[i].as_ptr(),
                messages[i].len(),
        ...
        for i in 0..n {
            let verify_result = pqcrystals_dilithium3_ref_verify(
                sigs[i].as_ptr(),
                siglens[i],
                messages[i].as_ptr(),
                messages[i].len(),
